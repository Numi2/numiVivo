...
The [matched Kang raw-count-to-graph run](../Tools/Omics/Reduction/MetalDistanceBlocks/EndToEnd/README.md)
adds fresh native CPU/Metal and exact Scanpy timing, memory and graph-equivalence
...
The earlier [donor-selected experiment](../Tools/Omics/CountObservation/Joint/Adaptive/Full/DonorExclusion/Prediction/ResponseShrinkage/README.md), [two-study transfer failure](../Tools/Omics/CountObservation/Joint/Adaptive/Full/DonorExclusion/Prediction/ResponseShrinkage/CrossStudy/README.md), and [range diagnostic](../Tools/Omics/CountObservation/Joint/Adaptive/Full/DonorExclusion/Prediction/ResponseShrinkage/CrossStudy/Support/README.md) remain available with their original feature sets and denominators. Failures persist inside observed training ranges, so range membership is not a validated accuracy gate. The current shared response model remains insufficient for study-context transfer; further donor-only tuning or a passing-subset claim would not resolve that gap.
...
| Preserve biology through integration | Complete HIRISA execution and coarse response checks pass. Native annotation retention fails 37/146 supported, sensitive comparisons, including 10/29 rare comparisons. The [condition-stratified candidate](../Tools/Omics/Benchmarks/HIRISA/CONDITION_STRATIFIED.md) improves mean sensitive recall but fails 40/146, including 10/29 rare, so it is not promoted. The [ridge-4 candidate](../Tools/Omics/Benchmarks/HIRISA/RIDGE4.md) leaves the native 37/146 and 10/29 failures unchanged. | The rigid projection still fails 31/146; its [label breakdown](../Tools/Omics/Benchmarks/HIRISA/RigidProjection/Diagnosis/README.md) includes 7 B intermediate and 5 CD14 Mono failures. The [translation-only ablation](../Tools/Omics/Benchmarks/HIRISA/TranslationOnly/README.md) still fails 22/146 annotation comparisons and one coarse response contrast. The [matched-control shift](../Tools/Omics/Benchmarks/HIRISA/MatchedControlShift/README.md) also fails 34/146 and increases donor scatter above original PCA. Resolve preservation losses and validate independently; no candidate is promoted. |
...
| Can proteomic, metabolomic or spatial-imaging values predict an outcome? | Requires declared assay units, sample/observation identity, a model linking those values to the requested endpoint and held-out measured outcomes. | Native quantitative JSON and H5MU interchange preserves these inputs, including missing values and measured zeros; fingerprint-bound coverage, finite-moment summaries and explicit pairwise complete-case associations are available, but no assay-specific outcome model or held-out validation is available. |
...
the original guide-to-control mapping remains unresolved. A source-count
cross-check supports a provisional `pBA582`/NegCtrl-3 and `pBA580`/NegCtrl-2
correspondence, but it is not an author construct mapping and does not clear
the role gate.
...
The follow-up [condition-stratified integration candidate](../Tools/Omics/Benchmarks/HIRISA/CONDITION_STRATIFIED.md)
fits donor effects within each declared condition and passes native replay plus
the independent SVD oracle. It improves aggregate sensitive recall relative to
the original PCA but fails 40/146 supported comparisons (10/29 rare), compared
with 37/146 for the existing native correction. This is a rejected integration
candidate and does not establish preservation or any phenotype outcome.
...
loss. It is rejected for promotion and does not change the conclusion that
general biological-outcome prediction is unestablished.
...
### Parse experimental provenance update: 2026-09-12

The [primary supplementary workbook](../Tools/Omics/PerturbationPrediction/ParseIFNB/BCellCounts/dose-provenance.json)
resolves the previously missing IFN-beta identity: 100 ng/mL, Biotechne / R&D
Systems, catalogue 8499-IF-010/CF. This supersedes earlier missing-dose statements
