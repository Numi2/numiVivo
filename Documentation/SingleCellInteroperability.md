...
10. **Metal:** the first explicit [sparse count-normalization backend](../Tools/Omics/CountStore/Metal/README.md)
    ...
    An opt-in `backend: .metalFP32` NB2 profile now carries the mean-dependent
    line-search objective through cohort Wald and LRT fits on a physical Apple
    GPU. A cached kernel uses a stable Poisson-limit series; count-only log-gamma
    terms, coefficient updates, diagnostics and the published `logLikelihood`
    remain on the exact FP64 CPU owner. Gene-wise dispersion profiles, trend
    fitting and MAP shrinkage remain CPU, while adjusted QL rejects this backend
    rather than falling back. The [native NB objective receipt](../Tools/Omics/Reduction/MetalDistanceBlocks/NativeOwner/evidence/2026-09-14-nb-objective/)
...
11. **Other omics:** a [SEQC technical bulk-RNA benchmark](../Tools/Omics/Benchmarks/SEQC/README.md)
    ...
    contigs and allele classes. `VivoVCFWriter` now provides a provenance-checked
    canonical export that reconstructs multiallelic records and sample fields;
    it is semantic rather than byte-identical and is re-hashed on reimport. This
