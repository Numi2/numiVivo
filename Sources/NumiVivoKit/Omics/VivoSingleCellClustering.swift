...
public struct VivoSingleCellClusteringOptions: Codable, Sendable, Equatable {
    public var resolution: Double = 1
    public var seed: UInt64 = 7
    public var maximumSweeps: Int = 100
    public var maximumLevels: Int = 32
    public var levelTolerance: Double = 1e-7
    public init() {}
    private enum CodingKeys: String, CodingKey { case resolution, seed, maximumSweeps, maximumLevels, levelTolerance }
    public init(from decoder: Decoder) throws {
        try vivoOmicsRejectUnknownKeys(decoder,allowed: ["resolution","seed","maximumSweeps","maximumLevels","levelTolerance"])
        let c = try decoder.container(keyedBy: CodingKeys.self)
        resolution = try c.decodeIfPresent(Double.self,forKey: .resolution) ?? 1
        seed = try c.decodeIfPresent(UInt64.self,forKey: .seed) ?? 7
        maximumSweeps = try c.decodeIfPresent(Int.self,forKey: .maximumSweeps) ?? 100
        maximumLevels = try c.decodeIfPresent(Int.self,forKey: .maximumLevels) ?? 32
        levelTolerance = try c.decodeIfPresent(Double.self,forKey: .levelTolerance) ?? 1e-7
    }
    public func validate() throws {
        guard resolution.isFinite,(0.01...100).contains(resolution), (1...1000).contains(maximumSweeps),
              (1...64).contains(maximumLevels),levelTolerance.isFinite,(0...1e-3).contains(levelTolerance) else {
            throw VivoOmicsError.invalid("clustering options")
        }
    }
}
...
/// Multilevel Louvain on the freshly constructed symmetric fuzzy graph.
/// Aggregated diagonal entries retain both directions of original internal edges.
enum VivoSingleCellClustering {
