...
/// An explicit allowlist of adapters to existing scientific authorities. Catalog
/// membership describes an executable interface, not a scientific certification.
public enum VivoPlatformOperations {
    ...
    /// Pure native adapter with exact reconstruction. Wrappers do not invent a
    /// second scientific implementation or weaken the wrapped method's checks.
    ...
    public static func registry(implementationFingerprint id: VivoFingerprint) throws -> VivoWorkflowRegistry {
        ...
        definitions.append(pure(identifier: "vivo.platform.target-reference", id: id,
            inputs: ["experiment": "vivo.target-engagement-experiment"], outputs: [.init(name: "result", kind: "vivo.target-engagement-result")],
            summary: "Exposure-driven target fractions with the existing FP64 reference; assumptions retained.",
        ...
        definitions.append(pure(identifier: "vivo.platform.finite-drug", id: id,
            inputs: ["experiment": "vivo.finite-drug-experiment"], outputs: [.init(name: "result", kind: "vivo.finite-drug-result")],
            summary: "Existing mass-balanced local finite-drug reaction operator; not an inferred pharmacological rate.", configure: empty,
