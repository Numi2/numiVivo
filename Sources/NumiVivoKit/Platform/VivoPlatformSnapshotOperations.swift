...
public enum VivoPlatformSnapshotOperations {
    /// Pure host mapping of an accepted checkpoint. The lineage continues to
    /// identify the canonical encoding of the decoded checkpoint; callers that
    /// retain original archive bytes must record their digest separately.
    public static func map(source: VivoMolecularStructureDocument, system: VivoClassicalSystem,
                           checkpoint: VivoMDCheckpoint, budget: VivoChemistryBudget = .init()) throws
        -> (document: VivoMolecularStructureDocument, frame: VivoTrajectoryFrame, mapping: VivoWorkflowSnapshotMapping) {
    ...
    public static func definition(implementationFingerprint id: VivoFingerprint) -> VivoWorkflowDefinition {
        VivoPlatformOperations.pure(identifier: "vivo.platform.md-snapshot", id: id,
            ...
            configure: VivoPlatformOperations.empty, calculate: { _, inputs, budget in
                ...
                let snapshot = try map(source: source, system: system, checkpoint: checkpoint, budget: budget)
                return ["structure": try snapshot.document.canonicalData(), "frame": try VivoCanonicalJSON.encode(snapshot.frame),
