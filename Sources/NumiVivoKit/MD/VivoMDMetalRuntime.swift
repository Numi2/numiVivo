...
public actor VivoMDMetalRuntime {
    ...
    public func step() async throws -> VivoMDStepCertificate {
        ...
        var abi=try VivoMDMetalABI.command(packed:packed,configuration:configuration,cell:periodicCell,stepIndex:acceptedStep)
        ...
        if !packed.constraints.isEmpty {
            ...
            try encode(.mdValidateConstraints,command:command,buffers:[arena.candidatePosition,arena.candidateVelocity,arena.constraints,arena.constraintOffsets,arena.constraintIncidence,arena.status],abi:abi)
        }
    ...
    public func snapshot() async throws -> VivoMDStateSnapshot {
        ...
        let value=VivoMDStateSnapshot(systemFingerprint:systemFingerprint,configurationFingerprint:configurationFingerprint,stepIndex:acceptedStep,timePS:acceptedTimePS,positionsNM:positions,velocitiesNMPerPS:velocities,periodicCell:periodicCell);try value.validate(particleCount:arena.particleCount);return value
    }
    ...
    private func encodePositionConstraints(_ command:MTLCommandBuffer,abi:VivoMDMetalCommand)throws{
        ...
        for _ in 0..<configuration.maximumConstraintIterations{try encode(.mdConstraintPosition,command:command,buffers:[source,destination,arena.dynamics,arena.constraints,arena.constraintOffsets,arena.constraintIncidence,arena.status],abi:abi);swap(&source,&destination)}
    ...
    private func encodeVelocityConstraints(_ command:MTLCommandBuffer,abi:VivoMDMetalCommand)throws{
        ...
        for _ in 0..<configuration.maximumConstraintIterations{try encode(.mdConstraintVelocity,command:command,buffers:[arena.candidatePosition,source,destination,arena.dynamics,arena.constraints,arena.constraintOffsets,arena.constraintIncidence,arena.status],abi:abi);swap(&source,&destination)}
