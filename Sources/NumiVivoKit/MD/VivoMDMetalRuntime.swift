...
/// One transaction owns positions, velocities, periodic geometry and the
/// cell-dependent neighbor/PME plans. No accepted state changes before the last
/// fallible operation in a dynamics step. Observation methods are read-only.
public actor VivoMDMetalRuntime {
    ...
    public static func restore(system: VivoClassicalSystem, configuration: VivoMDConfiguration,
        ...
        let systemID = try system.fingerprint(), configurationID = try configuration.fingerprint()
    ...
    private static func makeInternal(system: VivoClassicalSystem, initial: VivoClassicalInitialState,
        ...
        // Use the executed FP32 charges for every electrostatic correction.
        ...
        if configuration.ensemble == .npt { barostat = try await .make(device:device,catalog:catalog,system:system) }
        else { barostat = nil }
    ...
    private static func makeCellResources(device:MTLDevice,catalog:NumiVivoPipelineCatalog,
        ...
        if configuration.resolvedNeighborListEnabled, let cell {
            grid = try await .make(device:device,catalog:catalog,particleCount:packed.particleCount,cell:cell,
                neighborRadiusNM:configuration.cutoffNM+configuration.neighborSkinNM,neighborCapacity:arena.neighborCapacity)
        } else { grid = nil }
        let pme: VivoPMEEngine?
        if configuration.electrostatics == .pme {
            guard let cell else { throw VivoMDRuntimeError.unsupported(["PME requires a periodic cell"]) }
            pme = try await .make(device:device,catalog:catalog,particleCount:packed.particleCount,
    ...
    private init(system:VivoClassicalSystem,configuration:VivoMDConfiguration,device:MTLDevice,
        ...
        self.system=system;self.configuration=configuration;self.device=device;self.catalog=catalog;self.queue=queue
        self.packed=packed;self.arena=arena;self.work=work;self.pipelines=pipelines;totalChargeE=totalCharge
        cellResources=cell;barostatEngine=barostat;acceptedStep=step;acceptedTimePS=time
    ...
    public func step() async throws -> VivoMDStepCertificate {
        ...
        guard acceptedStep < .max, nextTime.isFinite, nextTime > acceptedTimePS else {
        ...
        try normalize(buffer,position:arena.candidatePosition,velocity:arena.candidateVelocity,abi:abi)
        // Rebuild after drift instead of selecting trajectories according to a
        // realized displacement/skin failure. Periodic construction uses cells.
        try neighbors(buffer,position:arena.candidatePosition,phase:phase,abi:abi)
        try forces(buffer,position:arena.candidatePosition,phase:phase,abi:abi)
        try encode(.mdHalfKick,buffer,[arena.candidateVelocity,arena.forceEnergy,arena.dynamics],abi)
        try projectVelocity(buffer,position:arena.candidatePosition,source:arena.candidateVelocity,
        ...
        guard status.flags==0 else { return certificate(committed:false,status:status,barostat:nil,timeAfter:acceptedTimePS) }
        ...
        try Task.checkCancellation()
        // Last fallible operation has finished. No await between the state/cell
        // ownership change and the corresponding accepted clock publication.
        let result=certificate(committed:true,status:.init(flags:0,firstParticle:.max,violationCount:0,reserved:0),
                               barostat:pressure,timeAfter:nextTime)
        arena.commit();cellResources=finalPhase;acceptedStep+=1;acceptedTimePS=nextTime
        return result
    }

    /// Explicit initialization, never an implicit effect of opening a checkpoint.
    /// Constraint projection yields tangent velocities; this does not rescale the
    /// realized kinetic energy to force an exact instantaneous temperature.
    public func thermalize(temperatureK:Double,seed:UInt64) async throws -> VivoMDCheckpoint {
        ...
        var abi=command(for:cellResources);abi.langevinA=0;abi.targetTemperatureK=Float(temperatureK)
        ...
        let buffer=try makeCommand("thermalize");try copyAccepted(into:buffer);try clear(buffer)
        try normalize(buffer,position:arena.candidatePosition,velocity:arena.candidateVelocity,abi:abi)
        try encode(.mdLangevin,buffer,[arena.candidateVelocity,arena.dynamics],abi)
        try projectVelocity(buffer,position:arena.candidatePosition,source:arena.candidateVelocity,
                            scratch:arena.velocityScratch,dynamics:arena.dynamics,abi:abi)
        ...
        arena.commit()
        return try checkpoint(from:await readSnapshotReserved())
    }
    ...
    public func minimize(_ settings:VivoMDMinimizationConfiguration = .init()) async throws -> VivoMDMinimizationCertificate {
        ...
        let phase=cellResources,abi=command(for:phase)
        // Geometry minimization explicitly starts from an admissible projected
        // configuration with zero velocities. Its first energy is that geometry.
        let start=try makeCommand("minimize.initialProjection");try copyAccepted(into:start);try clear(start)
        ...
        while attempts<settings.maximumIterations,baseline.maximumForce>settings.forceToleranceKJPerMolNM,
              scale>=settings.minimumStepScale {
            try Task.checkCancellation();attempts+=1
            let trial=try makeCommand("minimize.trial");try copyAccepted(into:trial);try clear(trial)
            ...
            if readStatus().flags==0 {
                do { candidate=try await evaluate(position:arena.candidatePosition,velocity:arena.candidateVelocity,phase:phase,gradient:true) }
            ...
            if let candidate,candidate.energy<baseline.energy {
                ...
                baseline=try await evaluate(position:arena.acceptedPosition,velocity:arena.acceptedVelocity,phase:phase,gradient:true)
            }
        }
    ...
    public func checkpoint() async throws -> VivoMDCheckpoint {
        // Metadata comes from the SAME snapshot, not from actor fields read after
        // returning from an await that another caller can interleave with.
    ...
    -> (phase:VivoMDCellResources,certificate:VivoMDBarostatMoveCertificate) {
        guard let engine=barostatEngine,let oldCell=phase.cell,let t=configuration.targetTemperatureK,
        ...
        let dx=(2*uniform(seed:configuration.randomSeed,step:step,stream:0x4e505456)-1)*configuration.resolvedBarostatMaximumLogVolumeStep
        let scale=exp(dx/3),proposedCell=VivoPeriodicCell(a:oldCell.a*scale,b:oldCell.b*scale,c:oldCell.c*scale)
        func rejection(_ reason:String)->VivoMDBarostatMoveCertificate {
            .init(stepIndex:step,attempted:true,accepted:false,volumeBeforeNM3:oldVolume,volumeAfterNM3:oldVolume,
                  logVolumeDelta:dx,logAcceptanceRatio:-Double.greatestFiniteMagnitude,rejectionReason:reason)
        }
        ...
        // Operational allocation/compilation errors ABORT the uncommitted step;
        // they must never be converted into statistical Metropolis rejections.
        let proposalPhase=try await Self.makeCellResources(device:device,catalog:catalog,packed:packed,arena:arena,
            configuration:configuration,totalCharge:totalChargeE,cell:proposedCell)
        ...
        if readStatus().flags != 0 {
            ...
            do {
                ...
                let score=-(trial.energy-baseline.energy+pressure*(proposedVolume-oldVolume)*0.0602214076)/(0.00831446261815324*t)
                    +(Double(engine.plan.componentCount)+1)*dx
                ...
                result = .init(stepIndex:step,attempted:true,accepted:take,volumeBeforeNM3:oldVolume,
                    volumeAfterNM3:take ? proposedVolume:oldVolume,logVolumeDelta:dx,logAcceptanceRatio:score)
        ...
        // Accepted buffers and the accepted cell were never touched. Restore the
        // dynamics candidate, not a previously committed state.
        return(phase,result)
    }
    ...
    -> (energy:Double,maximumForce:Double) {
        ...
        if gradient {
            ...
            try projectVelocity(buffer,position:position,source:work.directionA,scratch:work.directionB,dynamics:work.unitDynamics,abi:abi)
    ...
    private func observationsReserved() async throws -> VivoMDObservables {
        ...
        let massive=UInt64(system.particles.filter{$0.massDa>0}.count),constraints=UInt64(system.constraints.count)
        guard massive*3>constraints else { throw VivoMDRuntimeError.metal("nonpositive constrained degrees of freedom") }
        let dof=massive*3-constraints,temperature=2*kinetic/(Double(dof)*0.00831446261815324)
    ...
    private func reduce(_ buffer:MTLCommandBuffer,sumBoth:Bool) throws {
        ...
        while count>1 {
            ...
            guard let pipeline=pipelines[kernel],let encoder=buffer.makeComputeCommandEncoder() else { throw VivoMDRuntimeError.metal("reduction encoder unavailable") }
            encoder.setComputePipelineState(pipeline.state);encoder.setBuffer(current,offset:0,index:0);encoder.setBuffer(scratch,offset:0,index:1)
    ...
    private func neighbors(_ buffer:MTLCommandBuffer,position:MTLBuffer,phase:VivoMDCellResources,abi:VivoMDMetalCommand) throws {
        ...
        if let grid=phase.neighbors {
            ...
            try encode(.mdBuildNeighborList,buffer,[position,arena.neighborCounts,arena.neighborIndices,arena.neighborReferencePosition,arena.status],abi)
        }
    }
    private func forces(_ buffer:MTLCommandBuffer,position:MTLBuffer,phase:VivoMDCellResources,abi:VivoMDMetalCommand) throws {
        ...
        if configuration.electrostatics == .pme {
            ...
            try pme.encodeReciprocal(commandBuffer:buffer,positions:position,dynamics:arena.dynamics,forceEnergy:arena.forceEnergy,status:arena.status)
        ...
        } else {try encode(configuration.resolvedNeighborListEnabled ? .mdNonbondedNeighbor:.mdNonbondedDirect,buffer,pairBuffers,abi)}
        if !packed.virtualSites.isEmpty {
            try encode(.mdRedistributeVirtualForce,buffer,[arena.forceEnergy,arena.virtualSites,arena.virtualParentOffsets,arena.virtualParentIncidence,arena.status],abi)
        }
    }
    private func minimizePosition(_ buffer:MTLCommandBuffer,abi:VivoMDMetalCommand,scale:Double,maximum:Double) throws {
        ...
        defer{encoder.endEncoding()};encoder.setComputePipelineState(pipeline.state)
        encoder.setBuffer(arena.candidatePosition,offset:0,index:0);encoder.setBuffer(work.directionA,offset:0,index:1)
        encoder.setBuffer(arena.dynamics,offset:0,index:2)
        var md=abi,c=VivoMDMinPositionCommand(particleCount:packed.particleCount,stepScale:Float(scale),maxDisplacementNM:Float(maximum))
        encoder.setBytes(&md,length:MemoryLayout<VivoMDMetalCommand>.stride,index:3);encoder.setBytes(&c,length:16,index:4)
    ...
    private func readSnapshotReserved() async throws -> VivoMDStateSnapshot {
        let buffer=try makeCommand("snapshot")
        try copy(buffer,arena.acceptedPosition,arena.positionReadback)
        try copy(buffer,arena.acceptedVelocity,arena.velocityReadback)
        try await complete(buffer)
        ...
        try state.validate(particleCount:arena.particleCount);return state
    }
    ...
    private func certificate(committed:Bool,status:VivoMDMetalStatus,barostat:VivoMDBarostatMoveCertificate?,timeAfter:Double)->VivoMDStepCertificate {
        .init(systemFingerprint:systemFingerprint,configurationFingerprint:configurationFingerprint,deviceName:deviceName,
              ...
              committed:committed,statusFlags:status.flags,firstViolationParticle:status.firstParticle == .max ? nil:status.firstParticle,
    ...
    private func uniform(seed:UInt64,step:UInt64,stream:UInt64)->Double {
        ...
        // 52-bit midpoint grid is strictly inside (0,1) in binary64.
        return(Double(z>>12)+0.5)*0x1.0p-52
    }
    ...
    private func makeCommand(_ label:String)throws->MTLCommandBuffer {
        ...
        buffer.label="NumiVivo.MD.\(acceptedStep).\(label)";return buffer
    }
    private func copyAccepted(into buffer:MTLCommandBuffer)throws {
        try copy(buffer,arena.acceptedPosition,arena.candidatePosition);try copy(buffer,arena.acceptedVelocity,arena.candidateVelocity)
    }
    private func copy(_ buffer:MTLCommandBuffer,_ source:MTLBuffer,_ destination:MTLBuffer)throws {
        ...
        blit.copy(from:source,sourceOffset:0,to:destination,destinationOffset:0,size:arena.particleCount*16);blit.endEncoding()
    }
    private func clear(_ buffer:MTLCommandBuffer)throws {
        guard let pipeline=pipelines[.mdClearStatus],let encoder=buffer.makeComputeCommandEncoder() else {throw VivoMDRuntimeError.metal("clear encoder unavailable")}
    ...
    private func encode(_ kernel:NumiVivoKernel,_ buffer:MTLCommandBuffer,_ buffers:[MTLBuffer],_ abi:VivoMDMetalCommand)throws {
        guard let pipeline=pipelines[kernel],let encoder=buffer.makeComputeCommandEncoder() else {throw VivoMDRuntimeError.metal("missing encoder for \(kernel.rawValue)")}
        defer{encoder.endEncoding()};encoder.setComputePipelineState(pipeline.state);encoder.label=kernel.rawValue
        for(i,resource)in buffers.enumerated(){encoder.setBuffer(resource,offset:0,index:i)}
    ...
    private func ensureNumericalSuccess()throws {let status=readStatus();if status.flags != 0{throw VivoMDRuntimeError.candidateRejected(status.flags)}}
            ...
            try await withCheckedThrowingContinuation{(continuation:CheckedContinuation<Void,Error>)in
                buffer.addCompletedHandler{completed in
                    if completed.status != .completed {
                        continuation.resume(throwing:VivoMDRuntimeError.metal(completed.error.map{String(describing:$0)} ?? "command did not complete"))
