...
public struct VivoMDConfiguration: Codable, Sendable, Equatable {
    ...
    public init(timeStepPS: Double = 0.002, cutoffNM: Double = 1,
        ...
        self.neighborListEnabled = neighborListEnabled
        self.maximumNeighborsPerParticle = maximumNeighborsPerParticle
...
public struct VivoMDStateSnapshot: Codable, Sendable, Equatable {
    ...
    public init(systemFingerprint: VivoFingerprint, configurationFingerprint: VivoFingerprint,
        ...
        self.timePS = timePS; self.positionsNM = positionsNM
...
public enum VivoMDCapabilityAnalyzer {
    public static func analyze(system: VivoClassicalSystem,
        ...
        let resolvedVirtual = Set((system.linearVirtualSites ?? []).map(\.siteParticle))
        let unresolvedVirtual = virtual.map(\.index).filter { !resolvedVirtual.contains($0) }
        var blockers: [String] = []
        var notes: [String] = []
        if !unresolvedVirtual.isEmpty {
            blockers.append("virtual-site construction rules are unresolved for particles \(unresolvedVirtual)")
        } else if !virtual.isEmpty {
            notes.append("linear virtual-site reconstruction and parent-force redistribution are enabled")
        }
        if system.particles.contains(where: { $0.role == .drude }) {
            blockers.append("Drude polarization is not implemented")
        }
        ...
        if system.mixingRule == .explicitPairTable, pairs.isEmpty {
            blockers.append("explicit pair table is absent")
        }
        if configuration.resolvedNeighborListEnabled,
           UInt64(configuration.resolvedMaximumNeighborsPerParticle) > UInt64(max(system.particles.count - 1, 0)) {
        ...
        return .init(executable: blockers.isEmpty,
                     ...
                     virtualSiteCount: UInt32(virtual.count),
                     ...
                     explicitPairTypeCount: UInt32(pairs.count), blockers: blockers, notes: notes)
    }
}
