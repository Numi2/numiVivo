...
public struct VivoMDConfiguration: Codable, Sendable, Equatable {
    ...
    public init(timeStepPS: Double = 0.002, cutoffNM: Double = 1.0, neighborSkinNM: Double = 0.15,
        ...
        schema = Self.schema; self.timeStepPS = timeStepPS; self.cutoffNM = cutoffNM
        self.neighborSkinNM = neighborSkinNM; self.electrostatics = electrostatics
        self.relativeDielectric = relativeDielectric; self.reactionFieldDielectric = reactionFieldDielectric
        self.ensemble = ensemble; self.thermostat = thermostat; self.targetTemperatureK = targetTemperatureK
        self.frictionPerPS = frictionPerPS; self.barostat = barostat; self.targetPressureBar = targetPressureBar
        self.barostatInterval = barostatInterval; self.constraintTolerance = constraintTolerance
        self.maximumConstraintIterations = maximumConstraintIterations
        self.neighborRebuildInterval = neighborRebuildInterval; self.randomSeed = randomSeed
    }

    public func validate() throws {
        guard schema == Self.schema, timeStepPS.isFinite, timeStepPS > 0,
              cutoffNM.isFinite, cutoffNM > 0, neighborSkinNM.isFinite, neighborSkinNM >= 0,
              ...
              maximumConstraintIterations > 0, neighborRebuildInterval > 0, barostatInterval > 0 else {
        ...
        if ensemble == .npt || barostat != .none {
            guard ensemble == .npt, barostat != .none,
                  let p = targetPressureBar, p.isFinite, p > 0 else {
...
public struct VivoMDStateSnapshot: Codable, Sendable, Equatable {
    ...
    public init(systemFingerprint: VivoFingerprint, configurationFingerprint: VivoFingerprint,
                stepIndex: UInt64, timePS: Double, positionsNM: [VivoVector3D],
                velocitiesNMPerPS: [VivoVector3D], periodicCell: VivoPeriodicCell?,
                potentialEnergyKJPerMol: Double? = nil, kineticEnergyKJPerMol: Double? = nil,
                temperatureK: Double? = nil) {
        schema = Self.schema; self.systemFingerprint = systemFingerprint
        self.configurationFingerprint = configurationFingerprint; self.stepIndex = stepIndex
    ...
    public func validate(particleCount: Int) throws {
        guard schema == Self.schema, positionsNM.count == particleCount, velocitiesNMPerPS.count == particleCount,
              positionsNM.allSatisfy(\.isFinite), velocitiesNMPerPS.allSatisfy(\.isFinite),
              timePS.isFinite, timePS >= 0, periodicCell?.isValid != false,
              potentialEnergyKJPerMol?.isFinite != false, kineticEnergyKJPerMol?.isFinite != false,
...
public enum VivoMDCapabilityAnalyzer {
    public static func analyze(system: VivoClassicalSystem, initialState: VivoClassicalInitialState,
        ...
        var blockers: [String] = [], notes: [String] = []
        ...
        let pairs = system.nonbondedTypePairs ?? []
        if system.mixingRule == .explicitPairTable, pairs.isEmpty { blockers.append("explicit pair table is absent") }
        ...
        return .init(executable: blockers.isEmpty,
                     ...
                     virtualSiteCount: UInt32(virtualSites.count), bondCount: UInt32(system.bonds.count),
                     angleCount: UInt32(system.angles.count), torsionCount: UInt32(system.torsions.count),
                     constraintCount: UInt32(system.constraints.count), explicitPairTypeCount: UInt32(pairs.count),
