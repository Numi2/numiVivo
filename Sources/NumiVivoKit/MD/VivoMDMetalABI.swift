...
enum VivoMDMetalABI {
    ...
    static func command(packed: VivoMDPackedSystem,
        ...
        let mode: UInt32
        switch configuration.electrostatics { case .cutoff: mode=0; case .reactionField: mode=1; case .pme: mode=2 }
        ...
        if configuration.electrostatics == .reactionField {
            ...
            krf=Float((external-eps)/(2*external+eps)/pow(cutoff,3)); crf=Float(3*external/(2*external+eps)/cutoff)
        }
        ...
        return .init(particleCount:packed.particleCount,typeCount:packed.typeCount,electrostatics:mode,periodic:cell==nil ? 0:1,
                     stepLow:UInt32(truncatingIfNeeded:stepIndex),stepHigh:UInt32(truncatingIfNeeded:stepIndex>>32),
                     seedLow:UInt32(truncatingIfNeeded:configuration.randomSeed),seedHigh:UInt32(truncatingIfNeeded:configuration.randomSeed>>32),
