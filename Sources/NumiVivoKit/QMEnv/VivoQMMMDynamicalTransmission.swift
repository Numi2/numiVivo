...
public struct VivoQMMMTransmissionAcceptance: Codable, Sendable, Equatable {
    ...
    public init(minimumSurfaceCheckpoints:Int=32,minimumEffectiveFluxSamples:Double=20,
        ...
        self.maximumUnresolvedFluxFraction=maximumUnresolvedFluxFraction
        self.maximumCoefficientStandardError=maximumCoefficientStandardError
    }
...
public struct VivoQMMMDynamicalTransmissionRequest: Codable, Sendable, Equatable {
    ...
    /// Configuration under which surface checkpoints were produced. Its force
    /// settings must match the shooting Hamiltonian; it is normally the PMF NVT configuration.
    public var sourceDynamics:VivoMDConfiguration
    /// Unbiased propagation must be deterministic NVE. Only thermal controls may
    /// differ from sourceDynamics; force, PME, cutoff and constraint settings must match.
...
public enum VivoQMMMDynamicalTransmission {
    public static let interpretation="Flux-weighted classical dynamical transmission coefficient from product-directed, time-reversal-symmetrized dividing-surface checkpoints propagated under the same unbiased Born-Oppenheimer Hamiltonian in deterministic NVE. The result measures finite-time classical recrossing only; it is not a tunnelling correction or evidence for missing reaction pathways."
    ...
    private static func validate(_ request:VivoQMMMDynamicalTransmissionRequest,system:VivoClassicalSystem,
        ...
        let systemID=try system.fingerprint(),sourceID=try request.sourceDynamics.fingerprint()
        guard request.schema==VivoQMMMDynamicalTransmissionRequest.schema,
              ...
              request.sourceDynamics.ensemble == .nvt,request.sourceDynamics.targetTemperatureK==request.freeEnergy.analysis.temperatureK,
            ...
            throw VivoChemistryError.invalid("QM/MM dynamical-transmission Hamiltonian, dynamics or execution contract")
        }
        let coordinate=try VivoQMMMResolvedCoordinate(source:request.freeEnergy.analysis.coordinate,system:system)
        let direction=try productDirection(request)
        let sourceConfigID=try request.sourceDynamics.fingerprint()
        ...
        for checkpoint in request.surfaceCheckpoints {
            ...
            guard checkpoint.systemFingerprint==systemID,checkpoint.configurationFingerprint==sourceConfigID else {
                throw VivoChemistryError.invalid("surface checkpoint system or source dynamics mismatch")
            }
            let id=try checkpoint.fingerprint();guard ids.insert(id).inserted else {
                throw VivoChemistryError.invalid("duplicate dividing-surface checkpoint")
            }
    ...
    private static func orientedVelocity(coordinate:VivoQMMMResolvedCoordinate,checkpoint:VivoMDCheckpoint,
        ...
        let result=direction*velocity
        guard result.isFinite else { throw VivoChemistryError.invalid("nonfinite dividing-surface coordinate velocity") }
        return result
    }

    private static func reversed(_ checkpoint:VivoMDCheckpoint)->VivoMDCheckpoint {
    ...
    private static func transition(_ source:VivoMDCheckpoint,request:VivoQMMMDynamicalTransmissionRequest,
        ...
        let checkpoint=reverse ? reversed(staged.destinationCheckpoint):staged.destinationCheckpoint
    ...
    public static func run(_ request:VivoQMMMDynamicalTransmissionRequest,system:VivoClassicalSystem,
        ...
        let (coordinate,direction)=try validate(request,system:system,provider:baseProvider)
        let surface=request.freeEnergy.analysis.configuration.dividingSurfaceNM
        ...
        for source in request.surfaceCheckpoints {
            ...
            let rawVelocity=try orientedVelocity(coordinate:coordinate,checkpoint:source,direction:direction)
            ...
            if weight==0 {
                trajectories.append(.init(sourceCheckpointFingerprint:sourceID,initialCoordinateNM:surface,
                    productDirectedCoordinateVelocityNMPerPS:0,velocitiesTimeReversed:reverse,outcome:.unresolved,
                    committedSteps:0,finalCoordinateNM:surface));continue
            }
            ...
            var finalCoordinate=try coordinate.evaluate(.init(particlePositionsNM:start.positionsNM,periodicCell:start.periodicCell)).valueNM
            ...
            while committed<request.maximumSteps,outcome == .unresolved {
                ...
                if request.productCommitmentRangeNM.contains(finalCoordinate) {
                    productStreak+=1;reactantStreak=0
                } else if request.reactantCommitmentRangeNM.contains(finalCoordinate) {
                    reactantStreak+=1;productStreak=0
    ...
    public static func analyze(request:VivoQMMMDynamicalTransmissionRequest,systemFingerprint:VivoFingerprint,
        ...
        try request.acceptance.validate()
        guard trajectories.count==request.surfaceCheckpoints.count,trajectories.count>=request.acceptance.minimumSurfaceCheckpoints,
              trajectories.map(\.sourceCheckpointFingerprint).count==Set(trajectories.map(\.sourceCheckpointFingerprint)).count,
              trajectories.allSatisfy({$0.productDirectedCoordinateVelocityNMPerPS.isFinite && $0.productDirectedCoordinateVelocityNMPerPS>=0}) else {
            throw VivoChemistryError.invalid("dynamical-transmission trajectory evidence shape")
        }
        let weights=trajectories.map(\.productDirectedCoordinateVelocityNMPerPS),total=weights.reduce(0,+)
        let square=weights.reduce(0){$0+$1*$1}
        ...
        let stabilizedVariance=(coefficient*(1-coefficient)+1/(4*effective))/effective
        let standardError=sqrt(max(0,stabilizedVariance))
    ...
    public static func applying(_ result:VivoQMMMDynamicalTransmissionResult,
        ...
        output.transmissionProbability=result.transmissionCoefficient
        output.transmissionOrigin=.calculated
