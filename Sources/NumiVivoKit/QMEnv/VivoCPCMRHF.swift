...
public enum VivoCPCMRHF {
    public static func solve(system:VivoElectronicSystem,integrals ao:VivoAOIntegrals,
        ...
        guard system==ao.sourceSystem else{throw VivoChemistryError.invalid("C-PCM RHF source binding")}
        ...
        var x=overlapEigen.vectors;for i in 0..<n{for j in 0..<n{x[i,j]/=sqrt(overlapEigen.values[j])}}
        ...
        var history:[VivoCPCMDIISRecord]=[],trace:[VivoCPCMRHFIteration]=[],previous:Double?,densityChange=Double.infinity
        for iteration in 1...cfg.scf.maximumIterations {
            let gas=VivoHartreeFock.focks(ao,da,db),totalDensity=try da.adding(db)
            let pcm=try VivoCPCM.evaluate(system:system,integrals:ao,totalDensity:totalDensity,
                                          configuration:cfg.cpcm,budget:budget)
            ...
            let residual=sqrt(errors.reduce(0){$0+$1*$1})
            ...
            if let last=previous,abs(free-last)<=cfg.scf.energyToleranceHartree,
               densityChange<=cfg.scf.densityTolerance,residual<=cfg.scf.commutatorTolerance {
                ...
                let finalPCM=try VivoCPCM.evaluate(system:system,integrals:ao,totalDensity:try finalDA.adding(finalDB),
                                                   configuration:cfg.cpcm,budget:budget)
                let finalFock=try finalGas.0.adding(finalPCM.reactionPotentialMatrix)
                let finalErrors=try VivoHartreeFock.error(finalFock,finalDA,ao.overlap,x)
                    + VivoHartreeFock.error(finalFock,finalDB,ao.overlap,x)
                ...
                return .init(totalFreeEnergyHartree:finalFree,gasEnergyFunctionalHartree:finalGasEnergy,
                             polarizationEnergyHartree:finalPCM.polarizationEnergyHartree,
                             coefficients:coefficients,orbitalEnergiesHartree:orbitalEnergies,
                             alphaDensity:finalDA,betaDensity:finalDB,cpcm:finalPCM,
                             finalCommutatorNorm:finalResidual,iterations:trace,
                             status:"variational restricted HF + self-consistent C-PCM electrostatic reaction field; non-electrostatic solvent terms absent")
            }
            history.append(.init(fock:fa,error:errors));if history.count>cfg.scf.diisHistory{history.removeFirst()}
            var guess=fa
            if history.count>=2 {
            ...
            let (coefficients,_)=try canonical(guess);var next=VivoHartreeFock.density(coefficients,occupied:occupied)
            if iteration<=3{next=try next.scaled(1-cfg.scf.initialDamping).adding(da,scale:cfg.scf.initialDamping)}
            densityChange=2*(try next.adding(da,scale:-1).frobeniusNorm);previous=free;da=next;db=next
        }
