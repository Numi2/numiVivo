...
private struct VivoQMMMEnvironmentPosition: Codable, Sendable {
    let particleIndex: UInt32; let positionNM: VivoVector3D
}
...
public struct VivoQMMMLennardJonesResult: Codable, Sendable, Equatable {
    ...
    /// Legacy physical-QM view, in request order, with virtual-site forces folded.
    public let qmForcesHartreePerBohr: [VivoVector3D]
    /// Optional only for backward decoding of old results. New calculations always
    /// populate both fields. Full forces are raw site forces, NOT redistributed.
    public let qmAtomIndices: [UInt32]?
    public let particleForcesHartreePerBohr: [VivoVector3D]?
    public init(energyHartree: Double, qmForcesHartreePerBohr: [VivoVector3D],
                qmAtomIndices: [UInt32]? = nil, particleForcesHartreePerBohr: [VivoVector3D]? = nil) {
        self.energyHartree = energyHartree; self.qmForcesHartreePerBohr = qmForcesHartreePerBohr
        self.qmAtomIndices = qmAtomIndices; self.particleForcesHartreePerBohr = particleForcesHartreePerBohr
    }
}
...
public enum VivoQMMMCompiler {
    ...
    private static func mapping(document: VivoMolecularStructureDocument, system: VivoClassicalSystem,
        ...
        guard positions.count == system.particles.count, positions.allSatisfy(\.isFinite), !request.qmAtomIndices.isEmpty,
            ...
            throw VivoChemistryError.invalid("QM/MM coordinates or region request")
        }
        ...
        return topology.atomToParticle.map(Int.init)
    }
    ...
    public static func prepare(document: VivoMolecularStructureDocument, system: VivoClassicalSystem,
        ...
        let chemicalPairs = Set(document.structure.bonds.map { key($0.atomA, $0.atomB) })
        for bond in system.bonds {
            guard let a = system.particles[Int(bond.a)].atomIndex, let b = system.particles[Int(bond.b)].atomIndex else { continue }
            if selected.contains(a) != selected.contains(b), !chemicalPairs.contains(key(a,b)) {
                throw VivoChemistryError.unsupported("classical bond crosses QM/MM without a chemical bond-order record")
            }
        }
        ...
        for bond in crossing {
            guard bond.order == .single else {
                throw VivoChemistryError.unsupported("QM/MM boundary cuts must be explicit single bonds; promote the conjugated/metal region")
            }
            ...
            guard let distance = request.hydrogenLinkDistancesNM[symbol] else {
                throw VivoChemistryError.unsupported("no explicit hydrogen-link distance for \(symbol)")
            }
        ...
        let electronic = VivoElectronicSystem(nuclei: nuclei, alphaElectrons: request.alphaElectrons,
                                               betaElectrons: request.betaElectrons, pointCharges: charges)
        ...
        return .init(structureFingerprint: document.structureFingerprint, classicalSystemFingerprint: classicalFingerprint,
            snapshotFingerprint: snapshot, frozenEnvironmentFingerprint: environmentFingerprint, regionPolicyFingerprint: policyFingerprint,
            electronicSystem: electronic, qmAtomIndices: atoms, excludedQMParticles: excluded.sorted(), z1ZeroedMMParticles: zeroed.sorted(),
            links: links, energyConvention: "finite-cluster-electrostatic-Z1; E_QM+nucleus-MM+QM-MM-LJ; frozen-MM-self omitted; link atoms have no LJ")
    }

    /// Full cross-partition LJ energy and equal/opposite particle forces. Actual
    /// QM-owned virtual sites retain their LJ parameters; artificial links have none.
    public static func lennardJones(document: VivoMolecularStructureDocument, system: VivoClassicalSystem,
        ...
        let excluded = Set(prepared.excludedQMParticles)
        let qm = system.particles.filter { excluded.contains($0.index) }, mm = system.particles.filter { !excluded.contains($0.index) }
        let (work, overflow) = qm.count.multipliedReportingOverflow(by: mm.count)
        ...
        _ = try budget.elements([system.particles.count, 3], simultaneousArrays: 3)
        ...
        var energy = 0.0, forces = [VivoVector3D](repeating: .zero, count: system.particles.count)
        for q in qm { for m in mm {
            ...
            let force = delta * (scale * (12 * c12 * inverse12 - 6 * c6 * inverse6) * inverse2)
            forces[Int(q.index)] = forces[Int(q.index)] + force
            forces[Int(m.index)] = forces[Int(m.index)] - force
        } }
    ...
    /// Source-compatible legacy entry point, now using strict molecular closure.
    public static func promoteSolventResidues(document: VivoMolecularStructureDocument, system: VivoClassicalSystem,
        ...
        _ = try mapping(document: document, system: system, positions: positions, request: request)
        return try VivoQMMMSolventPromotion.select(document: document, system: system, positions: positions, base: request,
            policy: .init(radiusNM: radiusNM, solventResidueNames: solventResidueNames.sorted(), distanceAtoms: .allAtoms,
                          periodicMinimumImage: false), distanceCell: nil, requiresUnwrapping: false).qmAtomIndices
    }
}
