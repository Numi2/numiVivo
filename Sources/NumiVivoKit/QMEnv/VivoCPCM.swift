...
public struct VivoCPCMCavityConfiguration: Codable, Sendable, Equatable {
    ...
    public func validate(system: VivoElectronicSystem) throws {
        guard (26...4096).contains(pointsPerAtom), radiusScale.isFinite, radiusScale>0, maximumTesserae>0,
...
public enum VivoCPCM {
    ...
    public static func cavity(system:VivoElectronicSystem,
        ...
        try system.validate(); try cfg.validate(system:system)
        let directions=sphere(cfg.pointsPerAtom)
        let radii=try system.nuclei.map { nucleus -> Double in
            ...
            let radius=cfg.radiusScale*a/angstromPerBohr
            guard radius.isFinite, radius>0 else { throw VivoChemistryError.invalid("C-PCM radius overflow") }
            return radius
        }
        ...
        for (atom,nucleus) in system.nuclei.enumerated() {
            ...
            guard area.isFinite, area>0, selfTerm.isFinite, selfTerm>0 else { throw VivoChemistryError.invalid("C-PCM tessera area") }
            for direction in directions {
                let point=nucleus.positionBohr+radius*direction
                guard vivoQMFinite(point) else { throw VivoChemistryError.invalid("C-PCM surface coordinate overflow") }
                let buried=system.nuclei.indices.contains { other in
                    other != atom && vivoQMNorm(point-system.nuclei[other].positionBohr)<radii[other]-cfg.burialToleranceBohr
                }
    ...
    static func applySurfaceMatrix(_ x:[Double], _ t:[VivoCPCMTessera]) throws -> [Double] {
        ...
        for i in t.indices {
            ...
            for j in 0..<i {
                ...
                guard r.isFinite, r>1e-10 else { throw VivoChemistryError.invalid("coincident C-PCM tesserae") }
    ...
    static func solve(rhs:[Double], tesserae:[VivoCPCMTessera], tolerance:Double,
                      maximumIterations:Int, budget:VivoChemistryBudget) throws -> ([Double],Double,Int) {
        let n=rhs.count
        guard rhs.allSatisfy(\.isFinite) else { throw VivoChemistryError.invalid("C-PCM surface right hand side") }
        let (perIteration,overflow)=n.multipliedReportingOverflow(by:n)
        guard !overflow, perIteration<=budget.maximumOperatorApplications else { throw VivoChemistryError.resourceLimit("C-PCM surface-operator work") }
        // Reserve one application for a final true-residual measurement.
        let availableIterations=min(maximumIterations,budget.maximumOperatorApplications/max(1,perIteration)-1)
        guard availableIterations>0 else { throw VivoChemistryError.resourceLimit("C-PCM surface-solve work budget") }
        var x=[Double](repeating:0,count:n), r=rhs
        var z=zip(r,tesserae).map { $0.0/$0.1.selfInteractionPerBohr }, p=z, rz=dot(r,z)
        let rhsNorm=rhs.reduce(0.0) { hypot($0,$1) }
        if rhsNorm==0 { return (x,0,0) }
        guard rz.isFinite, rz>0 else { throw VivoChemistryError.convergence("C-PCM initial PCG residual") }
        for iteration in 1...availableIterations {
            let ap=try applySurfaceMatrix(p,tesserae), denominator=dot(p,ap)
            guard denominator.isFinite, denominator>0 else { throw VivoChemistryError.convergence("C-PCM surface matrix is not positive definite under current tessellation") }
            let alpha=rz/denominator
            for i in 0..<n { x[i]+=alpha*p[i]; r[i]-=alpha*ap[i] }
            let residual=r.reduce(0.0) { hypot($0,$1) }
            if residual<=tolerance*rhsNorm { return (x,residual,iteration) }
            z=zip(r,tesserae).map { $0.0/$0.1.selfInteractionPerBohr }
            let next=dot(r,z)
            guard next.isFinite, next>0 else { throw VivoChemistryError.convergence("C-PCM PCG breakdown") }
            let beta=next/rz
            for i in 0..<n { p[i]=z[i]+beta*p[i] }; rz=next
        }
        if availableIterations<maximumIterations { throw VivoChemistryError.resourceLimit("C-PCM aggregate surface-solve work budget") }
    ...
    public static func evaluate(system:VivoElectronicSystem, integrals ao:VivoAOIntegrals, totalDensity:VivoQMMatrix,
        ...
        try VivoCPCMOperator(system:system,integrals:ao,configuration:cfg,budget:budget).evaluate(totalDensity:totalDensity)
    }
}
