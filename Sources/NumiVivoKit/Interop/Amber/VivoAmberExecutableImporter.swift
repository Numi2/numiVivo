...
public enum VivoAmberExecutableImporter {
    public static func importSystem(prmtopData: Data,
        ...
        guard !virtualParticles.isEmpty else { return result }
        guard let initial = result.initialState else {
        ...
        for residue in residuePointers.indices {
            ...
            // Wave B v1 deliberately recognizes only the common linear four-site
            // water family: one massless charge point + O/H/H physical parents.
            ...
            let weights = [1 - alpha, alpha * 0.5, alpha * 0.5]
            let rule = VivoLinearVirtualSite(
        ...
        guard accounted == virtualSet else {
            ...
            throw VivoArtifactValidationError.incompatible(
                "AMBER virtual particles remain unresolved: \(missing)"
            )
        }
