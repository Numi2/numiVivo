...
struct VivoSingleCellCLICommands {
    static func handles(_ name: String?) -> Bool {
        ...
        return ["singlecell-h5ad-programs", "singlecell-h5ad-programs-verify", "singlecell-target-kernel-fit", "singlecell-target-kernel-predict", "singlecell-target-kernel-verify", "singlecell-target-kernel-prediction-verify", "singlecell-pca-integrate", "singlecell-pca-integrate-verify", "singlecell-graph-embed", "singlecell-graph-embed-verify", "singlecell-graph-cluster", "singlecell-graph-cluster-verify", "singlecell-pca-neighbors", "singlecell-pca-neighbors-verify", "singlecell-h5ad-pca-query", "singlecell-h5ad-pca-query-verify", "singlecell-h5ad-pca", "singlecell-h5ad-pca-verify", "singlecell-h5ad-preflight", "singlecell-h5ad-store", "singlecell-count-store-verify", "singlecell-count-store-normalize", "singlecell-count-store-normalize-verify", "multiassay-h5mu-import", "multiassay-h5mu-write", "multiassay-10x-import", "multiassay-visium-import", "multiassay-verify", "singlecell-composition-prepare", "singlecell-composition-fit", "singlecell-composition-predict", "singlecell-composition-verify", "singlecell-composition-prediction-verify", "singlecell-perturbation-fit", "singlecell-perturbation-predict", "singlecell-perturbation-verify", "singlecell-perturbation-prediction-verify", "singlecell-reference-fit", "singlecell-reference-map", "singlecell-reference-verify", "singlecell-reference-map-verify", "singlecell-h5ad-project", "singlecell-h5ad-project-selection", "singlecell-h5ad-project-verify", "singlecell-h5ad-pseudobulk", "singlecell-h5ad-pseudobulk-verify", "singlecell-h5ad-annotate", "singlecell-h5ad-import", "singlecell-h5ad-write", "singlecell-run", "singlecell-verify", "singlecell-export", "singlecell-mex", "singlecell-help", "singlecell-example",
    ...
    func run(arguments: [String]) async -> Int32 {
        do {
            ...
            if command == "singlecell-h5ad-preflight" {
                guard arguments.count == 2 else { throw VivoOmicsError.invalid("singlecell-h5ad-preflight <source.h5ad>") }
                try printJSON(VivoSingleCellH5AD.preflightCountMatrix(URL(fileURLWithPath: arguments[1]))); return 0
            }
            ...
            if command == "quantitative-h5mu-import" {
                guard arguments.count == 6, arguments[2] == "--plan", arguments[4] == "--output" else {
                    throw VivoOmicsError.invalid("quantitative-h5mu-import <source.h5mu> --plan <plan.json> --output <new-bundle>")
                }
                let planBytes = try VivoSingleCellCampaignIO.readDocument(URL(fileURLWithPath: arguments[3]), maximumBytes: 2_097_152)
                let plan = try VivoCanonicalJSON.decode(VivoH5MUQuantitativePlan.self, from: planBytes)
                try printJSON(VivoQuantitativeH5MUIO.importH5MU(source: URL(fileURLWithPath: arguments[1]),
                    plan: plan, implementation: VivoWorkflowCLIImplementation.fingerprint(),
                    to: canonicalURL(URL(fileURLWithPath: arguments[5])))); return 0
            }
            if command == "quantitative-h5mu-verify" {
                guard arguments.count == 2 else { throw VivoOmicsError.invalid("quantitative-h5mu-verify <bundle>") }
                try printJSON(VivoQuantitativeH5MUIO.verify(URL(fileURLWithPath: arguments[1]),
                    implementation: VivoWorkflowCLIImplementation.fingerprint())); return 0
            }
            if command == "quantitative-assay-summary" {
                guard arguments.count == 4, arguments[2] == "--output" else {
                    throw VivoOmicsError.invalid("quantitative-assay-summary <dataset.json> --output <summary.json>")
                }
                let source = URL(fileURLWithPath: arguments[1])
                let output = try canonicalURL(URL(fileURLWithPath: arguments[3]))
                guard !FileManager.default.fileExists(atPath: output.path) else {
                    throw VivoOmicsError.invalid("quantitative assay summary output already exists")
                }
                let bytes = try VivoSingleCellCampaignIO.readDocument(source, maximumBytes: 536_870_912)
                let dataset = try VivoCanonicalJSON.decode(VivoQuantitativeAssayDataset.self, from: bytes)
                let summary = try dataset.descriptiveSummary()
                // The existence check above makes the atomic replacement safe;
            ...
            if command == "singlecell-h5ad-project-selection" {
                guard arguments.count==6,arguments[2]=="--selection-plan",arguments[4]=="--output" else {
                    throw VivoOmicsError.invalid("singlecell-h5ad-project-selection <source.h5ad> --selection-plan <pseudobulk-plan.json> --output <new-directory>")
                }
                let selectionBytes=try VivoSingleCellCampaignIO.readDocument(URL(fileURLWithPath: arguments[3]),maximumBytes: 4*1_024*1_024)
                let selectionPlan=try VivoCanonicalJSON.decode(VivoH5ADPseudobulkPlan.self,from: selectionBytes)
                try selectionPlan.validate()
                // External plan files may use harmless formatting or omit a
                // decoded default. Bind the normalized, validated plan bytes
                // that are actually carried into the projection receipt.
                let canonicalSelectionPlan=try VivoCanonicalJSON.encode(selectionPlan)
                guard let selection=selectionPlan.cellSelection else {
                    throw VivoOmicsError.invalid("selection plan must contain a source-bound cellSelection")
                }
                let countStorePlan=try VivoCanonicalJSON.encode(selectionPlan.mapping)
                let projection=try VivoH5ADProjectionPlan(selectedCells: selection,
                    selectionPlanFingerprint: VivoCanonicalJSON.fingerprint(canonicalSelectionPlan),
                    countStorePlanFingerprint: VivoCanonicalJSON.fingerprint(countStorePlan),
                    maximumOutputBytes: 2_147_483_648)
                let companions=try VivoH5ADProjectionCellSelectionArtifacts(selectionPlan: canonicalSelectionPlan,countStorePlan: countStorePlan)
                try printJSON(VivoH5ADProjection.publish(source: URL(fileURLWithPath: arguments[1]),plan: projection,selectionArtifacts: companions,
                    implementation: VivoWorkflowCLIImplementation.fingerprint(),to: canonicalURL(URL(fileURLWithPath: arguments[5]))));return 0
            }
            ...
            if command == "singlecell-logcpm-feature-verify" {
                guard arguments.count == 2 else { throw VivoOmicsError.invalid("singlecell-logcpm-feature-verify <bundle> < records.bin") }
                ...
                let receiptBytes = try VivoSingleCellCampaignIO.readDocument(root.appendingPathComponent("receipt.json"), maximumBytes: 16_384)
                let receipt = try JSONDecoder().decode([String: String].self, from: receiptBytes)
                guard receipt["schemaVersion"] == "1", receipt["format"] == "feature-major-group-f64-le-v1" else { throw VivoOmicsError.invalid("feature bundle format") }
                let planBytes = try VivoSingleCellCampaignIO.readDocument(root.appendingPathComponent("plan.json"), maximumBytes: 268_435_456)
                let summaryBytes = try VivoSingleCellCampaignIO.readDocument(root.appendingPathComponent("summary.json"), maximumBytes: 268_435_456)
                guard hex(SHA256.hash(data: planBytes)) == receipt["planSHA256"], hex(SHA256.hash(data: summaryBytes)) == receipt["summarySHA256"] else { throw VivoOmicsError.invalid("feature bundle metadata digest") }
                let plan = try VivoCanonicalJSON.decode(Plan.self, from: planBytes)
                let saved = try JSONDecoder().decode(VivoStreamedLogCPM.FeatureStreamSummary.self, from: summaryBytes)
                guard plan.ordering == .featureMajor, receipt["features"] == String(plan.featureIDs.count), receipt["groups"] == String(plan.groupIDs.count),
                      !plan.groupIDs.isEmpty, plan.groupIDs.count <= 4096, plan.featureIDs.count <= 16_000_000 / plan.groupIDs.count else { throw VivoOmicsError.invalid("feature bundle dimensions") }
                let meansURL = root.appendingPathComponent("means.bin")
                let attributes = try FileManager.default.attributesOfItem(atPath: meansURL.path)
                let expectedBytes = plan.featureIDs.count * plan.groupIDs.count * 8
                guard attributes[.type] as? FileAttributeType == .typeRegular,
                      (attributes[.size] as? NSNumber)?.intValue == expectedBytes,
                      receipt["meansBytes"] == String(expectedBytes) else { throw VivoOmicsError.invalid("feature bundle payload size or file type") }
                let means = try FileHandle(forReadingFrom: meansURL)
                defer { try? means.close() }
                var inputHash = SHA256(), meansHash = SHA256(), emitted = 0
                let reconstructed = try VivoStreamedLogCPM.consumeFeatures(featureIDs: plan.featureIDs, groupIDs: plan.groupIDs, rowGroups: plan.rowGroups, rowTotals: plan.rowTotals, read: {
                    ...
                    guard feature == emitted else { throw VivoOmicsError.invalid("feature bundle sequence") }
                    let expected = values.count * 8
                    var bytes = Data()
                    while bytes.count < expected {
                        let part = try means.read(upToCount: expected - bytes.count) ?? Data()
                        guard !part.isEmpty else { throw VivoOmicsError.invalid("truncated feature bundle") }
                        bytes.append(part)
                    }
                    meansHash.update(data: bytes)
                    try bytes.withUnsafeBytes { raw in
                        for index in values.indices {
                            let bits = UInt64(littleEndian: raw.loadUnaligned(fromByteOffset: index * 8, as: UInt64.self))
                            guard bits == values[index].bitPattern else { throw VivoOmicsError.invalid("feature mean does not reconstruct") }
                        }
                    }
                    emitted += 1
                })
                guard reconstructed == saved, emitted == plan.featureIDs.count,
                      (try means.read(upToCount: 1) ?? Data()).isEmpty,
                      hex(inputHash.finalize()) == receipt["inputSHA256"], hex(meansHash.finalize()) == receipt["meansSHA256"] else { throw VivoOmicsError.invalid("feature bundle reconstruction or digest differs") }
                try printJSON(["status": "verified-feature-stream", "features": String(emitted), "groups": String(plan.groupIDs.count)])
                return 0
            }
            ...
            if command == "singlecell-logcpm-stream" {
                ...
                guard expected.utf8.count == 64, expected.utf8.allSatisfy({ (48...57).contains($0) || (97...102).contains($0) }) else {
                    throw VivoOmicsError.invalid("expected lowercase SHA256")
                }
                let output = try canonicalURL(URL(fileURLWithPath: arguments[6]))
                guard !FileManager.default.fileExists(atPath: output.path) else { throw VivoOmicsError.invalid("output already exists") }
                let bytes = try VivoSingleCellCampaignIO.readDocument(URL(fileURLWithPath: arguments[2]), maximumBytes: 268_435_456)
    ...
    NumiVivo native single-cell workflows
      singlecell-h5ad-preflight <source.h5ad>
    ...
    multiassay-h5mu-import <source.h5mu> --plan <mapping.json> --output <new-bundle>
      ...
      singlecell-logcpm-feature-verify <bundle> < records.bin
      ...
      singlecell-h5ad-project-selection <source.h5ad> --selection-plan <pseudobulk-plan.json> --output <new-directory>  (writes count-store-plan.json)
