...
struct VivoCellResponseCLICommands {
    ...
    cell-response-train <corpus> --store <count-store> --plan <training-plan.json> --output <new-model>
    ...
    func run(arguments: [String]) async -> Int32 {
        do {
            ...
            case "cell-response-verify":
                ...
                return 0
            case "cell-response-train":
                guard arguments.count == 8, arguments[2] == "--store", arguments[4] == "--plan", arguments[6] == "--output" else {
                    throw VivoOmicsError.invalid(Self.help)
                }
                ...
                try printJSON(VivoCellResponseLearning.train(corpus: reader, plan: plan, implementation: implementation,
                                                              to: try canonicalURL(arguments[7])))
                return 0
