...
public enum NumiVivoKernel: String, CaseIterable, Sendable {
    ...
    case mdMinimizeReduceStage = "nvivo_md_minimize_reduce_stage"
    case mdMinimizePosition = "nvivo_md_minimize_position"
    case mdZeroVelocity = "nvivo_md_zero_velocity"
    case mdBarostatCenters = "nvivo_md_barostat_centers"
    ...
    case mdDriftConstraintImpulse = "nvivo_md_drift_constraint_impulse"
    case mdProjectedForceSeed = "nvivo_md_projected_force_seed"
    case mdValidateProjectedForce = "nvivo_md_validate_projected_force"
    case mdObservationTerms = "nvivo_md_observation_terms"
    case mdSumPairReduce = "nvivo_md_sum_pair_reduce"

    /// The manifest is public so the CLI can report exactly which source module
    /// supplies an entry point, without claiming that the GPU compiled it.
    public var sourceModule: String {
        ...
        case .mdMinimizeTerms, .mdMinimizeReduceStage, .mdMinimizePosition, .mdZeroVelocity:
            return "NumiVivoMDMinimization"
        case .mdBarostatCenters, .mdBarostatScale:
            return "NumiVivoMDBarostat"
        case .mdDriftConstraintImpulse, .mdProjectedForceSeed, .mdValidateProjectedForce,
             .mdObservationTerms, .mdSumPairReduce:
            return "NumiVivoMDExecutionSupport"
...
public struct NumiVivoPipeline: @unchecked Sendable {
    ...
    public func threadgroupSize(for elementCount: Int, preferred: Int? = nil) -> MTLSize {
        ...
        return .init(width: max(1, capped >= width ? (capped / width) * width : capped), height: 1, depth: 1)
    }
...
private final class NumiVivoRuntimeLibraryCache: @unchecked Sendable {
    ...
    func library(device: MTLDevice, module: String) throws -> MTLLibrary {
        ...
        let key = "\(device.registryID)/\(module)/v2"
