...
constexpr std::uint32_t rotateRight(std::uint32_t value, std::uint32_t amount) noexcept {
...
std::array<std::uint8_t, kFingerprintBytes>
sha256(std::span<const std::byte> bytes) noexcept {
    ...
    for (std::size_t index = 0; index < 8; ++index) {
        tail[paddedBytes - 1 - index] = static_cast<std::uint8_t>(bitLength >> (index * 8U));
    }
    ...
    std::array<std::uint8_t, kFingerprintBytes> digest{};
...
std::string hexFingerprint(
    const std::array<std::uint8_t, kFingerprintBytes>& fingerprint) {
    static constexpr char hexadecimal[] = "0123456789abcdef";
    std::string output;
    output.resize(kFingerprintBytes * 2);
    for (std::size_t index = 0; index < fingerprint.size(); ++index) {
        output[index * 2] = hexadecimal[fingerprint[index] >> 4U];
        output[index * 2 + 1] = hexadecimal[fingerprint[index] & 0x0FU];
    }
    return output;
}
