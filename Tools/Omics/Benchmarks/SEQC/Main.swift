...
struct Output: Encodable {
    let rowSemantics = "technical-library-preparations; no biological donor replication"
...
@main struct Main {
    static func main() throws {
        guard CommandLine.arguments.count==3 else { throw VivoOmicsError.invalid("seqc-technical-library REPORT_GZ REQUEST_JSON") }
        ...
        try source.metadata.validate();try source.pseudobulk.matrix.validate()
        guard source.metadata.countUnit == .fragmentCount, source.pseudobulk.countUnit == .fragmentCount else { throw VivoOmicsError.invalid("SEQC paired-fragment count units") }
