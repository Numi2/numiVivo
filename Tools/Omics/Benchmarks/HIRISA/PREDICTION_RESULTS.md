...
This is empirical prediction of a known perturbation in an unseen donor from its observed control, using experimental enrichment populations rather than verified single-cell annotations. The separate [PBMC preparation-transfer split](CONTEXT_TRANSFER.md) is now frozen, with independently verified sparse counts; its native aggregation and response predictions remain pending. The results do not establish unseen perturbation identity, calibrated uncertainty, causal mechanisms or general biological validity. Metadata/report residency remains a limitation. Subsequent complete PCA/graph and seed-7 integration results are linked from the benchmark README; full-cohort clustering is now verified, while broader biological preservation remains open.
