#!/usr/bin/env python3
"""Archive bounded integration IO, native regressions and exact solver trajectories."""
...
def main():
    ...
    roots={'integration-tiled':a.root}
    ...
    assert '14 tests in 4 suites passed' in (root/'admission-tests.log').read_text()
    ...
    probe=read(root/'probe/results.json');assert probe['status']=='passed' and probe['allSelectedValueBitsVerified']
    assert probe['matrixRows']==1612594 and probe['tiledValueBytes']==6553600
    ...
    for label,root in roots.items():
        for source in sorted(root.rglob('*')):
            ...
            name=label+'/'+str(rel)
            if source.name in ['numivivo','storage-probe','legacy-solver','current-solver'] or (source.suffix=='.h5ad' and source.stat().st_size>16777216):
    ...
    manifest=dict(schemaVersion=2,records=records,fullSources=full,externalPayloads=external,archiveScriptSHA256=sha(Path(__file__)),reuseManifests=reused,
        scope='Bounded production integration gather/scatter, exact current file versus previous resident solver trajectories across multiple tiles, full Kang/Hagai native numerical regressions and replay, fitted/query lifecycle and tampering controls. Complete HIRISA dimensions are covered by a synthetic storage access probe only. No full-HIRISA integration, biological preservation, Metal or controlled end-to-end performance qualification.')
