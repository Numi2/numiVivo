#!/usr/bin/env python3
"""Archive sequential ridge passes, full-cohort regressions and full-shape phase measurements."""
...
def main():
    ...
    roots={'integration-ridge-stream':a.root}
    ...
    assert '19 tests in 4 suites passed' in (root/'admission-tests.log').read_text()
    ...
    phase=read(root/'phase-probe/checks.json');probe=phase['results']
    assert phase['status']=='passed' and probe['status']=='passed'
    assert probe['cells']==1612594 and probe['clusters']==100 and probe['components']==20
    assert probe['allStatisticBitsExact'] and probe['allCorrectedBitsExact'] and probe['scratchRemoved']
    streamed=read(root/'real-streamed-probe-v2/checks.json');assert streamed['status']=='passed'
    assert {(c['cohort'],c['seed']) for c in streamed['cases']}=={(name,seed) for name in ['kang','hagai'] for seed in [7,19,41]}
    assert all(c['allMatrixAndDiagnosticBitsExact'] and c['streamedRidgePath'] for c in streamed['cases'])
    assert read(root/'real-streamed-probe/kang-status.json')['returnCode']==-5
    solver=read(root/'solver-probe/checks.json');assert solver['status']=='passed' and solver['blockRows']>solver['maximumTileRows']
    assert len(solver['cases'])==2 and all(c['allMatrixBitsExact'] and c['allDiagnosticBitsExact'] for c in solver['cases'])
    ...
    for label,root in roots.items():
        for source in sorted(root.rglob('*')):
            ...
            name=label+'/'+str(rel)
            if source.name in ['numivivo','probe','legacy-solver','current-solver'] or (source.suffix=='.h5ad' and source.stat().st_size>16777216):
    ...
    manifest=dict(schemaVersion=2,records=records,fullSources=full,externalPayloads=external,archiveScriptSHA256=sha(Path(__file__)),reuseManifests=reused,
        scope='Sequential ridge statistics and correction with unchanged scalar addition/subtraction order. Nineteen native tests, complete Kang/Hagai native replay and forced-multiwindow trajectories, fixed/adaptive multiple-tile solver proof, fitted/query and MNN controls. Full HIRISA shape phase measurements are synthetic arithmetic/storage evidence, not end-to-end performance or biological qualification. First grouped-metadata harness rejection preserved.')
