...
def main():
 ...
 lines += ['## Evidence and limits','',
  ...
  '', 'This is empirical prediction of a known perturbation in an unseen donor from its observed control, using experimental enrichment populations rather than verified single-cell annotations. The separate [PBMC preparation-transfer experiment](CONTEXT_TRANSFER.md) now completes all 120 frozen folds, with independent numerical checks and native replay. The results do not establish unseen perturbation identity, calibrated uncertainty, causal mechanisms or general biological validity. Metadata/report residency remains a limitation. Subsequent complete PCA/graph and seed-7 integration results are linked from the benchmark README; full-cohort clustering is verified, while broader biological preservation remains open.',
  '', 'The retrospective [donor-score sensitivity audit](DONOR_SCORE_SENSITIVITY.md) retains all 79 folds. Ridge beats no-change in 14 contrast means, with 13 improvements surviving every single-donor score omission. Its four mean-baseline wins include three stable improvements under this diagnostic. This is sensitivity of existing scores, without refitting or predictive interval estimation.',
