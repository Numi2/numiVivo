...
p.add_argument('--variant',choices=['native','native-fixed'],default='native');a=p.parse_args();root=a.root
...
audit=read(root/'receipt.json');assert sha(root/'counts.npz')==audit['countsSHA256']
with np.load(root/'counts.npz') as archive:
    all_y=archive['counts'];sample_ids=archive['sampleIDs'].tolist();feature_ids=archive['featureIDs'].tolist()
...
for case in manifest:
    if a.variant=='native-fixed' and case['method']=='ql':continue
    directory=root/a.variant/(case['cohort']+'-'+case['method']);receipt_path=directory/'receipt.json'
    ...
    if r is not None:
        ...
        assert d['controlReplicates']==d['treatmentReplicates']==len(meta['sampleIDs'])//2
        assert [o['donorID'] for o in d['observations']]==meta['donorIDs']
        y=all_y[[sample_ids.index(sample) for sample in meta['sampleIDs']]].T.astype(float)
        ...
        if rows:
            ...
            reported=[j for j,g in enumerate(rows) if features[g].get('log2FoldChange') is not None]
            check('reported-log2-effect',[features[rows[j]]['log2FoldChange'] for j in reported],effect[reported]/np.log(2),2e-12,True)
            if case['method']!='ql':
                check('reported-log2-SE',[features[rows[j]]['standardError'] for j in reported],se[reported]/np.log(2),2e-10,True)
                positions={g:j for j,g in enumerate(rows)}
                check('reported-Wald-z',[features[g]['zStatistic'] for g in tested],
                    [effect[positions[g]]/se[positions[g]] for g in tested],2e-10,True)
            elif ql.get('averageQLDispersion'):
                check('QL-scaled-dispersion',phi.ravel(),
                    [diagnostics[g]['trendDispersion']/ql['averageQLDispersion'] for g in rows],2e-12,True)
        ...
        if null_rows:
            ...
            size=1/phi
            null_ll=(gammaln(yy+size)-gammaln(size)-gammaln(yy+1)+yy*np.log(phi*null_mu)-(yy+size)*np.log1p(phi*null_mu)).sum(axis=1)
            check('null-NB-log-likelihood',[v['nullLogLikelihood'] for v in lr],null_ll,2e-6,True)
            delta=mu-null_mu
    ...
    result=dict(cohort=case['cohort'],method=case['method'],checkerSHA256=checker_sha,outputSHA256=receipt['outputSHA256'],
        ...
        scope='Count/design identity and conditional numerical arithmetic; QL tails use reported denominator DF. This experimental study does not establish FDR, power or interval coverage.')
...
expected_cases=48
assert a.variant=='native'
summary=dict(cases=len(summaries),status='passed' if len(summaries)==expected_cases and all(s['status']=='passed' for s in summaries) else 'partial-or-failed',
    results=[{k:v for k,v in s.items() if k!='checks'} for s in summaries],checkerSHA256=checker_sha)
(root/(a.variant+'-model-check-'+('available' if a.available else 'summary')+'.json')).write_text(json.dumps(summary,sort_keys=True,indent=2)+'\n')
