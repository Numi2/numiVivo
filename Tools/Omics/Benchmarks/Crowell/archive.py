...
for path in sorted(a.root.rglob('*')):
    ...
    large_external = (path.suffix in {'.h5ad', '.Rda'} or
    ...
    if large_external or author_source:
        ...
        if rel.name == 'report.json.gz':
            ...
            item['remotePath'] = '/Users/n/numivivo-crowell-20260910/'+str(rel)
        external.append(item)
        continue
    data = path.read_bytes()
    compress = path.suffix in {'.tsv', '.log', '.txt'}
...
(a.out/'manifest.json').write_text(json.dumps(dict(entries=entries, external=external,
    qualification='Exact tables and logs; full source matrices and native reports remain external. Original strict model-check failure retained.',
