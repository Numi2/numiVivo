...
import argparse,json,time,warnings,gc,gzip
...
try:
 ...
 with gzip.open(Path(__file__).parent/'bundle/result.json.gz') as labels_file: expected_labels=json.load(labels_file)['labels']
 np.testing.assert_array_equal(atlas.obs['numivivo_graph_community'].to_numpy(),expected_labels)
 del expected_labels;gc.collect()
 ...
 report={'status':'passed-complete-backed-open-and-annotations','source':str(args.source),'annotated':str(args.annotated),'cells':n,'features':m,'matrixBackend':type(atlas.X).__module__+'.'+type(atlas.X).__name__,'allOriginalObservationAndFeatureColumnsChecked':checks,'allNativeCommunityLabelsExact':True,'sampledCountRows':selected,'sampledCountEntries':count,'anndata':version('anndata'),'pandas':version('pandas'),'h5py':version('h5py'),'seconds':time.time()-start,'scope':'Complete backed AnnData open and full obs/var metadata equivalence; count slicing is sampled here. Count slicing in this check is sampled; no full count comparison is claimed for this export. AnnData metadata remain resident; not a biological prediction or end-to-end speedup test.'};args.report.write_text(json.dumps(report,indent=2)+'\n');print('PASS',flush=True)
