...
shasum -a 256 "${FILES[@]}" "$ROOT/Tools/Omics/NegativeBinomial/LikelihoodRatio/Main.swift" > "$OUT/sources.sha256"
swiftc -swift-version 6 -O -parse-as-library -I "$ROOT/Sources/CNumiVivoZlib" \
  "${FILES[@]}" "$ROOT/Tools/Omics/NegativeBinomial/LikelihoodRatio/Main.swift" -o "$OUT/nb-lrt"
