...
def main():
    ...
    parser.add_argument('--remote-spool-root', type=Path)
    ...
    def execute(old_run):
        ...
        if args.remote_spool_root is None:
            ...
            assert args.remote_spool_root.is_absolute()
            transport = 'remote-file-then-gzip-over-ipv4'
            remote_file = args.remote_spool_root/case/method/'native.json'
            execution_path = dest/'remote-execution.json'
            expected_execution = dict(remoteFile=str(remote_file),binarySHA256=binary_sha,requestSHA256=sha(payload))
            if execution_path.exists():
                execution = read(execution_path)
                assert all(execution[k]==v for k,v in expected_execution.items())
            else:
                command = '\n'.join(['set -e', 'mkdir -p '+shlex.quote(str(remote_file.parent)),
                                     'set -C', '/usr/bin/time -l '+shlex.quote(args.binary)+' > '+shlex.quote(str(remote_file))])
                with (dest/'native.log').open('wb') as log:
                    process = subprocess.run(['ssh','-4','macmini',command],input=payload,stdout=log,stderr=log)
                execution = dict(**expected_execution,exitCode=process.returncode)
                execution_path.write_text(json.dumps(execution,sort_keys=True,indent=2)+'\n')
            # A failed transfer can retry this read without rerunning the native computation.
            exit_code = execution['exitCode']
            compressed = subprocess.check_output(['ssh','-4','macmini','gzip -c '+shlex.quote(str(remote_file))])
            raw = gzip.decompress(compressed)
        packed = gzip.compress(raw,mtime=0)
        (dest/'native.json.gz').write_bytes(packed)
        receipt = dict(case=case,method=method,exitCode=exit_code,transport=transport,
