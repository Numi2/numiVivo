#!/usr/bin/env python3
"""Qualify complete native adjusted QL inference on the frozen experimental families."""
...
for run in runs[:args.limit]:
    ...
    if not native.get('completed'):
        (dest/'reference.json').write_text(json.dumps(dict(status='failed-native',checks=[],nativeFailures=native['failures'])))
        (dest/'reference.log').write_text('Native family incomplete; no complete-family reference pass claimed.\n')
    else:
    ...
    receipt = dict(**identity,case=run['case'],method=run['method'],features=run['genes'],
        ...
        seconds=native['seconds'],testedFeatures=len(native.get('inference',{}).get('testedIndices',[])),
        status=reference['status'],referenceReturnCode=reference_returncode)
