...
@main struct Main {
 static func main() throws {
  ...
  let moments = (input.moments ?? []).map { v -> Result<VivoOmicsNBDevianceMoments> in
   do { return .init(value: try VivoOmicsNBResidualAdjustment.moments(mean: v.mean,dispersion: v.dispersion,relativeTolerance: v.relativeTolerance ?? 1e-10,maximumTerms: v.maximumTerms ?? 1_000_000),error: nil) }
  ...
  let residuals = (input.residuals ?? []).map { v -> Result<VivoOmicsNBAdjustedResiduals> in
   do { return .init(value: try VivoOmicsNBResidualAdjustment.adjustedResiduals(counts: v.counts,means: v.means,design: v.design,dispersion: v.dispersion,averageQuasiDispersion: v.averageQuasiDispersion),error: nil) }
   catch { return .init(value: nil,error: error.localizedDescription) }
  }
