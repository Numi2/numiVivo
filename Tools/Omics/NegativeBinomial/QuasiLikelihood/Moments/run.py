...
def execute(task):
 ...
 details=[];failures=[];terms=0;moments=0;maxLeverage=0.;maximumBound=0.
 for j,(out,ref) in enumerate(zip(output,reference['table'],strict=True)):
  ...
  bound=max(max(x['meanTruncationBound']/x['mean'],x['varianceTruncationBound']/x['variance']) for x in mm);maximumBound=max(maximumBound,bound)
