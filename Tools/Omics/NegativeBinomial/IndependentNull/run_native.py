...
p.add_argument('--output-subdirectory',choices=['native','native-fixed'],default='native')
...
out=root/a.output_subdirectory;out.mkdir(exist_ok=True)
lock=out/'driver.lock';fd=os.open(lock,os.O_CREAT|os.O_EXCL|os.O_WRONLY)
os.write(fd,str(os.getpid()).encode());os.close(fd)
try:
    for case in manifest:
        if a.output_subdirectory=='native-fixed' and case['method']=='ql':continue
