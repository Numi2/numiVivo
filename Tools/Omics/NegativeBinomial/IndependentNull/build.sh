...
MAIN="$ROOT/Tools/Omics/NegativeBinomial/IndependentNull/Main.swift"
shasum -a 256 "${FILES[@]}" "$MAIN" > "$OUT/sources.sha256"
swiftc -swift-version 6 -O -parse-as-library -I "$ROOT/Sources/CNumiVivoZlib" "${FILES[@]}" "$MAIN" -o "$OUT/independent-null"
