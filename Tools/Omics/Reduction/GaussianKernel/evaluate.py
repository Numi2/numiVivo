...
def main():
 ...
 a=p.parse_args();a.out.mkdir(parents=True,exist_ok=False);freeze=read(a.root/'complete.json');assert freeze['metricsRead'] is False and freeze['allOriginalAnchorBytesAndAssemblyStructuresExact'] and all(freeze['results'][c+'-tiled/'+n]==freeze['results'][c+'-tiled-replay/'+n] for c in ('hagai','kang','ding') for n in ('scores.bin','anchors.bin','report.json'));spec=read(a.spec);owner=Path(__file__).parent.parent
 bindings={}
 for c in ('hagai','kang','ding'):
  inp=Path(spec[c]['inputs']);ref=Path(spec[c]['reference']);old=read(ref/'checks.json');assert sha(inp)==old['inputSHA256'];assert sha(a.root/(c+'-tiled')/'scores.bin')==freeze['results'][c+'-tiled/scores.bin'];bindings[c]=dict(inputsSHA256=sha(inp),referenceChecksSHA256=sha(ref/'checks.json'),originalNativeChecksSHA256=sha(Path(spec[c]['originalNative'])/'checks.json'),scoresSHA256=sha(a.root/(c+'-tiled')/'scores.bin'))
 margin=read(Path(spec['protocol']))['gateMargins'];write(a.out/'execution-freeze.json',dict(createdUnix=time.time(),outputsFreezeSHA256=sha(a.root/'complete.json'),candidateBindings=bindings,protocolSHA256=sha(Path(spec['protocol'])),evaluatorSHA256=sha(Path(__file__)),dependencySHA256={n:sha(owner/n) for n in ('check_full_integration.py','evaluate_ding_integration.py')},margins=margin))
 results=[]
 for c in ('hagai','kang','ding'):
  out=a.out/c;out.mkdir();inputs=np.load(spec[c]['inputs'],allow_pickle=False);old=read(Path(spec[c]['reference'])/'checks.json');baseline=old['baseline'];x=inputs['scores'];np.testing.assert_array_equal(x,matrix(a.source/c/'pca/scores.bin',len(x),x.shape[1]));y=matrix(a.root/(c+'-tiled')/'scores.bin',len(x),x.shape[1]);assert y.shape==x.shape and np.isfinite(y).all();start=time.perf_counter()
  if c=='ding':
  ...
  originalNative=read(Path(spec[c]['originalNative'])/'checks.json')
  value=dict(originalNativeMetrics=originalNative['metrics'],originalNativeGates=originalNative['gates'],status='complete-original-cohort-measured',cohort=c,cells=len(x),baseline=baseline,metrics=m,gates=g,allMeasuredGatesPassed=all(g.values()),seconds=time.perf_counter()-start,scope='Native all-anchor tiled correction on inspected full cohorts with unchanged original diagnostics; unavailable/partial labels retained. Not independent biology or prospective prediction.');write(out/'checks.json',value);results.append(dict(cohort=c,cells=len(x),gates=g));print(c,g,flush=True)
