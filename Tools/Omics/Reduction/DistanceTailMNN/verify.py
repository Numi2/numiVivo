...
assert f['librarySHA256']==m['members']['libHybrid.dylib']
for n,h in out['files'].items():
 path='candidate/'+n
 assert h==(m['members'][path] if path in m['members'] else m['externalArrays'][path]['SHA256'])
tests=read('refinement-tests.json');assert tests['status']=='passed' and tests['librarySHA256']==m['members']['libHybrid.dylib']
assert tests['sourceSHA256']==m['members']['test_refinement.py']
ref=read('refinement-oracle.json');assert ref['scriptSHA256']==m['members']['check_refinement.py']
assert ref['outputsFreezeSHA256']==m['members']['candidate/outputs-frozen.json']
assert sum(x['refinedQueries'] for x in ref['results'])==6828
assert all(x['allSelectedExact'] and x['unselectedUnchanged'] and x['maximumDistanceError']==0 for x in ref['results'])
assert read('verification-attempt-1/checks-terminal.json')['returnCode']!=0
...
for x in s['cohorts']:
 ...
 n=next(v for v in read('numerical.json')['results'] if v['cohort']==x['cohort'])
 assert n['finalReconstructionExact'] and n['anchorPrecision']==x['anchorPrecision'] and n['anchorRecall']==x['anchorRecall']
