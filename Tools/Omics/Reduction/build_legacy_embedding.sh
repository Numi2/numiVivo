...
xcrun swiftc -swift-version 6 -O -parse-as-library -I "$LIB" -I "$ROOT/Sources/CNumiVivoZlib" -I "$ROOT/Sources/NumiVivoCore/include" -L "$LIB" -lNumiVivoKit \
  "$ROOT/Tools/Omics/Reduction/LegacyEmbeddingReference.swift" "$ROOT/Tools/Omics/Reduction/LegacyEmbeddingMain.swift" "$LIB/OmicsHNSW.o" "$LIB/OmicsGaussian.o" -framework Accelerate -lc++ -o "$OUT"
