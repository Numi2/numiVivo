...
def main():
 ...
 for c in ('hagai','kang','ding'):
  ...
  value=dict(status='complete-original-cohort-measured',cohort=c,cells=len(x),baseline=baseline,metrics=m,gates=g,allMeasuredGatesPassed=all(g.values()),seconds=time.perf_counter()-start,scope='Development local correction on inspected full cohorts with unchanged original diagnostics; unavailable/partial labels retained. Not independent biology or prospective prediction.');write(out/'checks.json',value);results.append(dict(cohort=c,cells=len(x),gates=g));print(c,g,flush=True)
