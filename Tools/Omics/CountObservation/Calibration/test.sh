...
swiftc -swift-version 6 -parse-as-library -F "$F" -Xfrontend -plugin-path -Xfrontend "$P" -Xlinker -rpath -Xlinker "$F" \
 ...
 "$OUT/OmicsHNSW.o" "$OUT/OmicsGaussian.o" -framework Accelerate -lc++ -o "$OUT/calibration-tests"
"$OUT/calibration-tests"
swiftc -swift-version 6 -O -parse-as-library -I "$OUT" -I "$ROOT/Sources/CNumiVivoZlib" -I "$ROOT/Sources/NumiVivoCore/include" -L "$OUT" -lNumiVivoKit \
 "$ROOT/Tools/Omics/CountObservation/Calibration/Train.swift" "$OUT/OmicsHNSW.o" "$OUT/OmicsGaussian.o" -framework Accelerate -lc++ -o "$OUT/calibrate-cells"

swiftc -swift-version 6 -O -parse-as-library -I "$OUT" -I "$ROOT/Sources/CNumiVivoZlib" -I "$ROOT/Sources/NumiVivoCore/include" -L "$OUT" -lNumiVivoKit \
 "$ROOT/Tools/Omics/CountObservation/Calibration/Apply.swift" "$OUT/OmicsHNSW.o" "$OUT/OmicsGaussian.o" -framework Accelerate -lc++ -o "$OUT/apply-calibration"
