...
with h5py.File(cache,'r') as h:
 library=[h[str(i)+'/libraryCounts'][:] for i in range(len(manifest['groups']))]
 pointers=[h[str(i)+'/indptr'][:] for i in range(len(library))]
     ...
     for position,line in enumerate(fo):
      ...
      if cp is None or tp is None:
       ...
       for gi,group in enumerate(manifest['groups']):
        lo,hi=map(int,pointers[gi][j:j+2]);g=h[str(gi)];y=np.zeros(len(library[gi]),np.uint32);y[g['indices'][lo:hi]]=g['data'][lo:hi]
