...
indices=[g['index'] for g in manifest['groups']];assert len(set(indices))==len(indices) and all(isinstance(i,int) and i>=0 for i in indices)
...
with h5py.File(cache,'r') as h:
 ...
 for gi,g in enumerate(manifest['groups']):
  group=h[str(manifest['groups'][gi]['index'])];assert group.attrs['donorID']==g['donorID'] and group.attrs['conditionID']==g['conditionID']
      ...
      for gene in manifest['genes'][first:last]:
       ...
       if gene['controlCellDispersion'] is not None and gene['treatedCellDispersion'] is not None:
        ...
        for gi,g in enumerate(groups):
         group=h[str(manifest['groups'][gi]['index'])];lo,hi=map(int,pointers[gi][j:j+2]);rows=group['indices'][lo:hi];values=group['data'][lo:hi]
