...
swiftc -swift-version 6 -parse-as-library -F "$F" -Xfrontend -plugin-path -Xfrontend "$P" -Xlinker -rpath -Xlinker "$F" \
 ...
 "$OUT/OmicsHNSW.o" "$OUT/OmicsGaussian.o" -framework Accelerate -lc++ -o "$OUT/adaptive-tests"
"$OUT/adaptive-tests"
swiftc -swift-version 6 -O -parse-as-library -I "$OUT" -I "$ROOT/Sources/CNumiVivoZlib" -I "$ROOT/Sources/NumiVivoCore/include" -L "$OUT" -lNumiVivoKit \
 "$ROOT/Tools/Omics/CountObservation/Joint/Adaptive/Run.swift" "$OUT/OmicsHNSW.o" "$OUT/OmicsGaussian.o" -framework Accelerate -lc++ -o "$OUT/adaptive-counts"
