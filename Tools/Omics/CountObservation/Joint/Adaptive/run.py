...
root=Path(sys.argv[1]);origin=sys.argv[2];remote='/Users/n/numivivo-adaptive-counts-20260911'
freeze=json.loads((root/'runtime-freeze.json').read_bytes());expected=freeze['adaptiveBinarySHA256']
native=remote+'/build/adaptive-counts'
...
save()
with (root/(origin+'-adaptive.stderr')).open('xb') as stderr:
 ...
 with (root/(origin+'-adaptive.jsonl.gz')).open('xb') as raw,gzip.GzipFile(filename='',fileobj=raw,mode='wb',mtime=0) as f:
