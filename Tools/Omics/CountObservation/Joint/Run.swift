...
struct Row: Encodable {
    ...
    let model: VivoJointCountModel?;let prediction: VivoJointCountPrediction?;let detail: String?
}
@main struct Main {
    static func main() throws {
        ...
        var failures=0
        for g in input.genes { for grid in input.gridSizes {
            try autoreleasepool {
                let id=input.origin+":"+g.featureID+":grid="+String(grid)
                var model: VivoJointCountModel?,status="unavailableCellDispersion"
                ...
                if let cp=g.controlCellDispersion,let tp=g.treatedCellDispersion {
                    do {
                        model=try VivoJointCountResponse.fit(pairs: g.pairs,featureID: g.featureID,controlConditionID: "control",treatedConditionID: "IFNB",
                            controlCellDispersion: cp,treatedCellDispersion: tp,trainingSource: source,plan: .init(gridPointsPerAxis: grid))
                ...
                for q in input.queries where q.featureID==g.featureID {
                    if let model,status=="convergedFiniteGridLikelihood" {
                        do {
                            let p=try VivoJointCountResponse.predict(control: q.control,featureID: q.featureID,queryDonorID: q.donorID,controlConditionID: "control",querySource: source,model: model,plannedTreatedLibraryCounts: q.plannedLibraryCounts)
