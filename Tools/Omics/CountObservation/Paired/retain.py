...
archive.unlink()
manifest={'schemaVersion':1,'format':'sha256-objects-with-logical-file-aliases','files':files,'objects':{h:{'bytes':len(b)} for h,b in objects.items()},'externalSources':external,'archive':{'bytes':archive_bytes,'SHA256':archive_hash,'parts':parts},'scope':'Complete paired reports and all donor omissions, independent references, full source moments and identities, exact executable, source snapshots, logs and failures; original H5AD qualification remains bound to the published parent archive.'}
