...
root=Path(sys.argv[1]);origin=sys.argv[2];remote='/Users/n/numivivo-paired-counts-20260911'
freeze=json.loads((root/'runtime-freeze.json').read_bytes());expected=freeze['pairedBinarySHA256']
native=remote+'/build/paired-counts'
...
with (root/(origin+'-native.stderr')).open('xb') as stderr:
 ...
 def send():
  try:
   ...
   expected_input=json.loads((root/'input-freeze.json').read_bytes())['origins'][origin]['inputSHA256']
