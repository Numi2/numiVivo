#!/usr/bin/env python3
"""Freeze all 150 nested-selected inputs, then execute native predictor and replay.
...
import sys
sys.path.insert(0, '/Users/n/numivivo-replogle2020-20260911')
...
def main():
    ...
    assert selection['protocolSHA256'] == sha(Path(__file__).parent/'protocol.json')
    assert selection['selectorSHA256'] == sha(Path(__file__).parent/'select_regularization.py')
    assert selection['sourceInputFreezeSHA256'] == sha(root/'target-kernel/input-freeze.json')
    chosen = {(x['gemgroup'],x['target']): x for x in selection['folds']}
    ...
    for count_fold in frozen_counts['folds']:
        ...
        assert chosen[gem,target]['selectedTrainingSHA256'] == hashlib.sha256(encoded(selected)).hexdigest()
        ...
        plan = dict(schemaVersion=1, context=training['selection']['context'], descriptorNamespace=namespace,
            source=dict(bytes=list(bytes.fromhex(annotation_sha))), regularization=chosen[gem,target]['selectedRegularization'], maximumWork=200000000,
            provenance='Replogle development: nested training-target-only lambda selection, fixed full-gene normalization and GO identity. Outer target excluded from inner selection; technical gemgroup only.',
    ...
    write(out/'input-freeze.json', dict(frozenUTC=datetime.datetime.now(datetime.timezone.utc).isoformat(),
        selectionSHA256=sha(selection_path), protocolSHA256=sha(Path(__file__).parent/'protocol.json'), binarySHA256=BINARY, driverSHA256=sha(Path(__file__)), countSelectorSHA256=sha(Path(__file__).with_name('prepare_training.py')),
