...
struct Kernel {
 ...
 init(_ controls:[[Double]],_ studies:[String]) {
  let n=controls.count,g=controls[0].count;w=weights(studies)
  var mean=[Double](repeating:0,count:g)
  for i in 0..<n {for j in 0..<g {mean[j]+=w[i]*controls[i][j]}}
  center=mean;x=controls.map {row in zip(row,mean).map(-)}
  var k=Array(repeating:Array(repeating:0.0,count:n),count:n)
  for i in 0..<n {for j in 0...i {
   var dot=0.0;for c in 0..<g {dot+=x[i][c]*x[j][c]}
   let value=sqrt(w[i]*w[j])*dot/Double(g);k[i][j]=value;k[j][i]=value
  }}
  gram=k
 }
 func responseWeights(_ queries:[[Double]],_ penalty:Double?) throws -> ([[Double]],Double) {
  ...
  for q in queries {
   ...
   for i in 0..<n {var dot=0.0;for j in 0..<g {dot+=x[i][j]*(q[j]-center[j])};rhs[i]=sqrt(w[i])*dot/Double(g)}
   var a=rhs
